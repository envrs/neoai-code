...
local function register_events(on_init)
	...
	chat_binary:register_event("init", function(_, answer)
		...
		if config.is_enterprise() then init.serverUrl = config.get_config().neoai_enterprise_host end
		answer(init)
	...
	chat_binary:register_event("get_user", function(_, answer)
		neoai_binary:request({ State = { dummy = true } }, function(state)
			answer({
	...
	chat_binary:register_event("browse_file_v2", function(_, answer)
		vim.ui.input({
			...
			if vim.fn.executable("file") == 1 then
				mime_type = vim.fn.system("file --mime-type -b " .. vim.fn.shellescape(path)):gsub("%s+", "")
			else
			...
			-- Handle binary content with compatibility fallbacks
			local binary_content, base64_content
			if vim.fn.has("nvim-0.9.0") == 1 and vim.fn.readblob then
				binary_content = vim.fn.readblob(path)
				if vim.base64 and vim.base64.encode then
					base64_content = vim.base64.encode(binary_content)
				else
					-- Fallback: read as text and encode
					local content = vim.fn.readfile(path, "b")
					if vim.fn.executable("base64") == 1 then
						base64_content = vim.fn.system("base64", table.concat(content, "\n")):gsub("%s+", "")
					else
						-- Last resort: no base64 encoding available
						base64_content = table.concat(content, "\n")
					end
				end
			else
	...
	chat_binary:register_event("get_completions", function(request, answer)
		...
		neoai_binary:request({
			Autocomplete = {
				filename = "neoai-chat-input." .. file_extension,
...
function M.open(on_ready)
	if not M.enabled then
		vim.notify("Neoai Chat is available only for Pro users")
		return
	end

	if not chat_binary:available() then
		vim.notify(
			"neoai_chat binary not found, did you remember to build it first? `cargo build --release` inside `chat/` directory"
		)
		return
	end
...
function M.get_state()
    return chat_state or { window = nil, messages = {}, context = {} }
end

function M.send_message(message)
    M.submit_message(message)
end
