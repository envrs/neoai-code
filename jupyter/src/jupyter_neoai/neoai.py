...
class NeoaiDownloader(threading.Thread):
    def __init__(self, download_url, output_dir, neoai):
    ...
    def run(self):
        try:
            self.logger.info(
                "Begin to download Neoai Binary from %s", self.download_url
            )
            ...
            self.logger.info("Finish download Neoai Binary to %s", self.output_dir)
            sem_complete_on(self.neoai)
        except Exception as e:
            self.logger.error("Download failed, error: %s", e)


def sem_complete_on(neoai):
    SEM_ON_REQ_DATA = {
        "version": "1.0.7",
        "request": {
            "Autocomplete": {
    ...
    res = neoai.request(json.dumps(SEM_ON_REQ_DATA))
    try:
        neoai.logger.info(
            f' {res["results"][0]["new_prefix"]}{res["results"][0]["new_suffix"]}'
        )
    except Exception:
        neoai.logger.warning(" wrong response of turning on semantic completion")
...
class Neoai(object):
    ...
    def request(self, data):
        ...
        try:
            ...
            self.logger.debug("Neoai output is corrupted: " + output)
    ...
    def _restart(self):
        ...
        if path is None:
            self.logger.error("no Neoai binary found")
            return
    ...
    def _get_running_neoai(self):
        ...
        if self._proc is not None and self._proc.poll():
            self.logger.error(
                "Neoai exited with code {}".format(self._proc.returncode)
            )
            self._restart()
        return self._proc
    ...
    def download_if_needed(self):
        if os.path.isdir(self._binary_dir):
            ...
            if neoai_path is not None:
                add_execute_permission(neoai_path)
                self.logger.info(
                    "Neoai binary already exists in %s ignore downloading",
                    neoai_path,
                )
    ...
    def _download(self):
        ...
        download_url = "{}/{}/{}/{}.zip".format(
            _NEOAI_SERVER_URL, version, distro, _NEOAI_EXECUTABLE
        )
        output_dir = os.path.join(self._binary_dir, version, distro)
        NeoaiDownloader(download_url, output_dir, self).start()


def get_neoai_version():
    version_url = "{}/{}".format(_NEOAI_SERVER_URL, "version")
