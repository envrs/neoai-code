...
export default class ChatViewProvider implements WebviewViewProvider {
  ...
  private init() {
    ...
    const disposable = this.chatWebviewView?.onDidChangeVisibility(() => {
      disposable?.dispose();
      this.onVisible("neoai-chat-visible");
    });
    this.onVisible("neoai-chat-inited");
  ...
  async focusChatInput() {
    void vscode.commands.executeCommand("workbench.view.extension.neoai");
  ...
  setWebviewHtml(webviewView: WebviewView): void {
    const reactAppPath = path.join(
      process.env.NEOAI_CHAT_DIR ?? this.extensionPath,
      "chat",
      "index.html"
    );
