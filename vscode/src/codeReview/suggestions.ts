...
import NeoaiComment from "./NeoaiComment";
...
export default async function addSuggestions(
  ...
  try {
    ...
    statusBarItem.text = "Fecthing Neoai code review suggestions...";
    statusBarItem.show();
    ...
    response.focus.forEach((focus) => {
      ...
      const thread = controller.createCommentThread(document.uri, line.range, [
        new NeoaiComment(focus.old_value, suggestion, document.languageId),
      ]);
