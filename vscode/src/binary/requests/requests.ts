...
export const neoAiProcess = new Binary();
...
export function initBinary(processRunArgs: string[] = []): Promise<void> {
  return neoAiProcess.init(processRunArgs);
}

export function resetBinaryForTesting(): void {
  void neoAiProcess.resetBinaryForTesting();
}
...
export function autocomplete(
  ...
  return neoAiProcess.request<AutocompleteResult | undefined | null>(
    {
...
export function getState(
  ...
  return neoAiProcess.request<State>({ State: content });
}
...
export function fireEvent(
  ...
  return neoAiProcess.request<EventResponse>({
    Event: content,
  });
}
...
export function deactivate(): Promise<unknown> {
  if (neoAiProcess) {
    return neoAiProcess.request({ Deactivate: {} });
  }

  Logger.error("No NeoAi process");

  return Promise.resolve(null);
}

export function uninstalling(): Promise<unknown> {
  return neoAiProcess.request({ Uninstalling: {} });
}
...
> {
  try {
    const result = await neoAiProcess.request<CapabilitiesResponse>(
      { Features: {} },
      20000
    );
...
export async function getChatCommunicatorAddress(
  ...
  let response = await neoAiProcess.request<ChatCommunicationAddressResponse>(
    request
  );

  // retry. Could happen in case of binary restart
  if (response === null) {
    response = await neoAiProcess.request<ChatCommunicationAddressResponse>(
      request
    );
  }
