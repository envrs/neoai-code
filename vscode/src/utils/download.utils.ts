...
async function downloadResource<T>(
  ...
  const ca = neoaiExtensionProperties.caCerts
    ? await readCaCerts(neoaiExtensionProperties.caCerts)
    : undefined;
    ...
    const request = getHttpModule(parsedUrl).request(
      {
        ...
        agent,
        rejectUnauthorized: !neoaiExtensionProperties.ignoreCertificateErrors,
        ca,
        headers: { "User-Agent": "NeoAi.neoai-vscode" },
        timeout: 30_000,
      },
      (response) => {
        if (
