      ...
      if (artifactUrl) {
        ...
        if (isNewerAlphaVersionAvailable(context, availableVersion)) {
          ...
          void showMessage({
            ...
            messageText: `NeoAi has been updated to ${availableVersion} version. Please reload the window for the changes to take effect.`,
            buttonText: "Reload",
            action: () =>
...
async function showNotificationForBetaChannelIfNeeded(
  ...
  const shouldShowMessage =
    isPreReleaseChannelSupported() &&
    (neoaiExtensionProperties.isVscodeInsiders ||
      isCapabilityEnabled(Capability.ALPHA_CAPABILITY)) &&
    !didShowMessage &&
    !neoaiExtensionProperties.isExtensionBetaChannelEnabled;
  ...
  await showMessage({
    ...
    messageText:
      "Do you wish to help Neoai get better? Enable Neoai's extension beta channel if so!",
    buttonText: "Open Settings",
    action: () =>
      void commands.executeCommand(
        "workbench.action.openSettings",
        "neoai.receiveBetaChannelUpdates"
      ),
  });
