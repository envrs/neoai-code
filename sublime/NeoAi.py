...
class DisableViewCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        self.view.settings().set("neoai-disabled", True)
        set_state({"State": {"state_type": "disable-view"}})

    def is_visible(self, *args):
        return is_v2 and not self.view.settings().get("neoai-disabled", False)


class EnableViewCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        self.view.settings().set("neoai-disabled", False)
        set_state({"State": {"state_type": "enable-view"}})

    def is_visible(self, *args):
        return is_v2 and self.view.settings().get("neoai-disabled", False)


class EnableNativeAutoCompleteCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        sublime.load_settings("NeoAi.sublime-settings").set(
            "native_auto_complete", True
        )
        sublime.save_settings("NeoAi.sublime-settings")
